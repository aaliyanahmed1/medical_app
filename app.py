...
def postprocess(segmentation: np.ndarray, logits: np.ndarray):
    probs = np.exp(logits)
...
def side_by_side(original_bgr: np.ndarray, annotated_bgr: np.ndarray) -> np.ndarray:
    ...
    canvas[:annotated_bgr.shape[0], original_bgr.shape[1]:original_bgr.shape[1]+annotated_bgr.shape[1]] = annotated_bgr
    return canvas
...
def _load(onnx_path: str, meta_path: str):
